...
This step is to smooth gene expression by gene order within each chromosome, which improves CNA signal continuity and suppresses gene-specific noise. The most important input arguments are:
...
The `FindTumor()` function identifies tumor cells based on CNA profiles when no prior tumor annotations are available.
...
The number of tumor subclones must be pre-specified. If there is no prior knowledge, we provide the function `suggest_k`, which recommends an appropriate value for K based on a Silhouette score that integrates both CNA and spatial information. The most important input arguments are:
...
- `alpha`: A number bewtween 0 and 1. Control the weight of CNA in clustering. 
...
### Spatial CNA inference and subclone detection

The `FindClone()` function performs spatial CNA inference and tumor subclone identification based on CNA patterns, enhanced by spatial regularization. After tumor cells are identified, this function clusters them into subclonal populations that share similar CNA states while also preserving spatial continuity.
...
The parameter `lambda` controls the strength of spatial smoothing, encouraging neighboring cells to remain in the same subclone if supported by CNA evidence. The number of subclones to infer is set by `K`, allowing flexible modeling of tumor heterogeneity.


```{r}
# Spatial CNA inference and tumor subclone detection
...
### Visualize CNA heatmap

Visualize subclonal CNA patterns by heatmap:

```{r}
# Visualize CNA heatmap
log.cna <- log2(obj@cna.data)
log.cna[log.cna > -0.5 & log.cna < 0.5] <- 0
