...
#' \itemize{
#'   \item `obj@cna.data`: optimized CNA matrix (cells × bins).
...
FindTumor <- function(
  ...
  cna <- t(obj@smoothed.data)
  cna <- as.matrix(cna)
  cna <- cna + 1e-6
  all_cells <- colnames(cna)
  
  cat("CNA matrix dimensions: ", nrow(cna), " x ", ncol(cna), "\n")
  ...
  norm_mat <- cna[, ref_ids, drop = FALSE]
  row_means <- rowMeans(norm_mat)
  
  spatial_location <- obj@location
  spatial_location <- spatial_location[match(colnames(cna), spatial_location$cell.id), ]
  
  B <- diag(x = row_means)
  colnames(B) <- row.names(cna)
  row.names(B) <- row.names(cna)
  
  V <- sweep(cna, 1, row_means, "/")
  V <- t(V)
  
  AMatrix <- createA(spatial_location)
  Bins <- colnames(V)
  cna.df <- getConcated(V, spatial_location)
  
  kmeans_result <- kmeansFunc_Initialize(cna.df[, Bins], K)
  cna.df$kmeans_cluster <- as.numeric(kmeans_result$kmeans)
  TotalClusters <- sort(unique(cna.df$kmeans_cluster))
  
  if (!requireNamespace("Matrix", quietly = TRUE)) {
    stop("Package 'Matrix' is required.")
  }
  
  countMatrix <- Matrix::Matrix(cna, sparse = TRUE)
  ...
  for(iter in 1:max_iter){
    ...
    cna.df$kmeans_cluster <- as.numeric(kmeans_result$kmeans)
    TotalClusters <- sort(unique(cna.df$kmeans_cluster))
    centers_old <- getMu(Bins, TotalClusters, cna.df)
  ...
  rownames(ResList$V) <- colnames(countMatrix)
  colnames(ResList$V) <- Bins
  obj@cna.data <- ResList$V
  
  loc <- obj@location
  loc <- loc[match(rownames(cna.df), loc$cell.id), ]
  
  cna_mat <- obj@cna.data
  norm_mat <- cna_mat[rownames(cna_mat) %in% ref_ids, , drop = FALSE]
  
  id1 <- cna.df$cellID[cna.df$kmeans_cluster == 1]
  id2 <- cna.df$cellID[cna.df$kmeans_cluster == 2]
  
  mat1 <- cna_mat[id1, , drop = FALSE]
  mat2 <- cna_mat[id2, , drop = FALSE]
  ...
  cna.df$tumor <- ifelse(cna.df$kmeans_cluster == tumor, "Tumor", "Other")
  
  cluster_df <- data.frame(
    cell.id = rownames(cna.df),
    x = loc$x,
    y = loc$y,
    cell.label = cna.df$tumor,
    row.names = rownames(cna.df)
  )
...
#' The input `obj` with:
#' \itemize{
#'   \item `@cna.data`: optimized CNA matrix (tumor cells × bins).
...
FindClone <- function(
  ...
  # 1) CNA matrix: transpose to (bins x cells) and add small epsilon to avoid zeros
  cna <- t(obj@smoothed.data)
  cna <- as.matrix(cna)
  eps <- 1e-6
  cna <- cna + eps
  all_cells <- colnames(cna)
  cat("cna matrix dimensions (bins x cells): ", nrow(cna), " x ", ncol(cna), "\n")
  ...
  B <- diag(x = row_means)
  colnames(B) <- rownames(cna)  # gene names
  rownames(B) <- rownames(cna)
  
  # Compute V = CNA / ref_mean (cells x bins)
  V <- sweep(cna, 1, row_means, "/")
  ...
  kmeans_result <- kmeansFunc_Initialize(cna.df[, Bins, drop = FALSE], K)
  cna.df$kmeans_cluster <- as.numeric(kmeans_result$kmeans)
  TotalClusters <- sort(unique(cna.df$kmeans_cluster))
  ...
  countMatrix <- Matrix::Matrix(cna, sparse = TRUE)     # bins x tumor_cells
  AMatrix     <- Matrix::Matrix(AMatrix, sparse = TRUE)
  ...
  # Write back CNA (cells x bins; tumor cells only)
  ...
  cluster_df <- data.frame(
    ...
    x = loc$x,
    y = loc$y,
    cell.label = paste("Clone", cna.df$kmeans_cluster),
    row.names = rownames(cna.df),
  ...
  cat("\n========================================\n")
  cat("CNA inference and tumor subclone detection completed!\n")
...
#' using the average silhouette on an MDS embedding derived from CNA data.
#' Optionally fuses spatial distances from \code{obj@location} with CNA distances
...
#' @param alpha \code{NULL} for CNA-only. If a number in \code{[0,1]},
#'   fuse CNA and spatial distances: \eqn{d = alpha * d_{CNA} + (1-alpha) * d_{spatial}} (default: 0.7).
...
suggest_k <- function(
  ...
  ## ---------- 0) Pull matrices ----------
  cna_full <- t(obj@smoothed.data)
  cna_full <- as.matrix(cna_full) + 1e-6
  all_cells <- colnames(cna_full)
  ...
  ## ---------- 2) Reference mean from FULL CNA (not tumor-only) ----------
  ref_mat  <- cna_full[, ref_ids, drop = FALSE]    # bins x ref
  row_means <- rowMeans(ref_mat)                   # length = bins
  
  ## ---------- 3) Restrict to tumor cells, then compute V (cells x bins) ----------
  cna_tumor <- cna_full[, tumor_ids, drop = FALSE] # bins x tumor
  V <- t(sweep(cna_tumor, 1, row_means, "/"))      # cells x bins
  ...
  ## ---------- 7) Distances: CNA-only or fused (CNA+spatial) ----------
  d_cna <- stats::dist(X_cna, method = "euclidean")
  if (!is.null(alpha)) {
    if (!is.numeric(alpha) || alpha < 0 || alpha > 1) stop("`alpha` must be in [0,1] or NULL.")
    d_sp  <- stats::dist(loc_sub[, c("x","y")], method = "euclidean")
    rng1 <- range(d_cna); d_cna_n <- (d_cna - rng1[1]) / (rng1[2] - rng1[1] + 1e-12)
    rng2 <- range(d_sp);  d_sp_n  <- (d_sp  - rng2[1]) / (rng2[2] - rng2[1]  + 1e-12)
    dmat <- alpha * d_cna_n + (1 - alpha) * d_sp_n
  } else {
    dmat <- d_cna
  }
