...
#'
#' @description
#' Draws a spatial scatter plot from \code{cna_obj@cluster}, coloring points by
#' clone label.
#' Optionally flips the axes with \code{coord_flip()}.
#'
#' @param cna_obj highSpaClone object.
...
#' @examples
#' \dontrun{
#' p <- spatialplot(cna_obj, point_size = 0.01, use_coord_flip = T, use_x_reverse = T,
#'                  title = "Tumor subclones")
#' print(p)
#' }
#'
#' @export
spatialplot <- function(cna_obj,
...
#' Chromosome-binned CNA heatmap with group annotations
#'
#' @description
#' Draw a CNA heatmap split by chromosomes and annotated by a sample/cell grouping
#' variable. Designed for objects that provide `cna.data` (cells × bins),
...
#'
#' @param cna.obj A list-like object with components:
#'   \itemize{
#'     \item \code{cna.data}: numeric matrix, rows = cells/spots, cols = bins.
...
#'   }
#' @param groupby Character scalar; column name in \code{cna.obj$annotation}
...
cna_heatmap <- function(
  ...
  # ---- checks & extraction ----
  if (is.null(cna.obj$cna.data) || is.null(cna.obj$annotation) || is.null(cna.obj$chr_pos)) {
    stop("`cna.obj` must contain `cna.data`, `annotation`, and `chr_pos`.")
  }
  cna_mat <- as.matrix(cna.obj$cna.data)
  if (is.null(rownames(cna_mat))) stop("`cna.obj$cna.data` must have rownames = cell IDs.")
  annotation_df <- cna.obj$annotation
  ...
  row_idx <- match(annotation_df$cell.id, rownames(cna_mat))
  valid   <- !is.na(row_idx)
  if (!any(valid)) stop("No annotation cell.id matched CNA rownames.")
  row_idx <- row_idx[valid]
  cna_mat <- cna_mat[row_idx, , drop = FALSE]
  group_vec <- annotation_df[[groupby]][valid]
  
  # order rows by group
  ord <- order(group_vec)
  cna_mat   <- cna_mat[ord, , drop = FALSE]
  sorted_grp <- group_vec[ord]
  
  # ---- chromosome splitting from chr_pos (0-based starts) ----
  chr_pos <- cna.obj$chr_pos
  ...
  if (auto_size) {
    nr <- nrow(cna_mat); nc <- ncol(cna_mat)
  ...
  if (isTRUE(save)) {
    ...
    if (!write_pdf && !write_png) {
      ...
      ComplexHeatmap::decorate_heatmap_body("CNA", .decorate_groups())
      grDevices::dev.off()
      # PNG
